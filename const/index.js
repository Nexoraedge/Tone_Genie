import { MousePointer , Keyboard ,CheckCircle,Wand2,Users,Globe, Sparkles, Github, X, Linkedin, Mail } from "lucide-react"
export const testimonials = [
  {
    name: "Hardik Jain",
    role: "Content Creator",
    content: "This is not an actual rating....Rate us and provide feedback so that we can shape ToneGinie perfectly.",
    rating: 4,
  },
  {
    name: "DevDhoni-ai",
    role: "Freelance Writer",
    content: "This is not an actual rating....Rate us and provide feedback so that we can shape ToneGinie perfectly.",
    rating: 5,
  },
  {
    name: "Hardik Jain",
    role: "Student",
    content: "This is not an actual rating....Rate us and provide feedback so that we can shape ToneGinie perfectly.",
    rating: 4,
  },
]

export const icons = [
              {
                icon: MousePointer,
                title: "Select Text",
                description: "Highlight any text on any website - emails, social media, documents, anywhere!",
              },
              {
                icon: Keyboard,
                title: "Press Shortcut",
                description: "Use CTRL + SHIFT + G to enhance or CTRL + SHIFT + O to customize tone",
              },
              {
                icon: Sparkles,
                title: "Get AI Magic",
                description: "Instantly receive perfectly rewritten text in your desired tone and style",
              },
            ]
 export const Powerful_features = [
              {
                icon: CheckCircle,
                title: "Grammar + Style Fixer",
                description: "Corrects grammar, refines tone, and boosts clarity.",
              },
              {
                icon: Wand2,
                title: "Custom Prompt Improviser",
                description: "Type anything like 'make this persuasive + witty' and let AI do the rest.",
              },
              {
                icon: Users,
                title: "Human-like Tone Output",
                description: "Polished, fluid, and natural-sounding responses.",
              },
              {
                icon: Globe,
                title: "Works on Any Website",
                description: "Fiverr, Gmail, social DMs, LinkedIn — anywhere you write.",
              },
            ]
export const forwhom = [
              { title: "Freelancers", emoji: "💼" },
              { title: "Content Creators", emoji: "✨" },
              { title: "Students", emoji: "🎓" },
              { title: "Email Writers", emoji: "📧" },
              { title: "Social Media Managers", emoji: "📱" },
              { title: "Copywriters", emoji: "✍️" },
              { title: "Bloggers", emoji: "📝" },
              { title: "Business Professionals", emoji: "👔" },
            ]
export const data = [
  {
    first:"CTRL",
    second:"SHIFT",
    third:"G",
    smallD:"Quick Enhancement 🚀",
    extra:"Instantly improve grammer , style , tone and clarity",
    initial:-50
  },
  {
    first:"CTRL",
    second:"SHIFT",
    third:"O",
    smallD:"Custom Tone ✒️",
    extra:"Choose specific tone, style preferences and provide context.",
initial:50
  },
  
  {
    first:"CTRL",
    second:"SHIFT",
    third:"H",
    smallD:"From Scratch 📜",
    extra:"Write the content you want with prompt and provided data",
    initial:-50
  },
  {
    first:"CTRL",
    second:"SHIFT",
    third:"A",
    smallD:"Extension 🧩",
    extra:"From here you can go the extension , cheeck out our website and other features",
    initial:50
  }
]
export const extralinks = [
  {
    href:"/feedback",
    text:"Feedback"
  },
  {
    href:"/privacy",
    text:"Privacy Policy"
  },
  {
    href:"/terms",
    text:"Terms"
  },
  {
    href:"/",
    text:"Contact"
  },
]
export const socaillinks =[
  {
    href:"#",
    icon:Github,

  },
  {
    href:"#",
    icon:X,

  },
  {
    href:"#",
    icon:Linkedin,

  },
  {
    href:"#",
    icon:Mail,

  },
]